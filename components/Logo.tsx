...
export function LogoMark({ className }: LogoMarkProps) {
  return (
    <svg
      viewBox="65 95 447 362"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
    >
      <g transform="translate(0,554) scale(0.1,-0.1)" fill="currentColor" stroke="none">
        <path
...
export default function Logo({ className, wordmarkClassName }: LogoProps) {
  return (
    <span className={`inline-flex items-center gap-2 ${className ?? ""}`}>
      ...
      <span
        className={`font-display text-xl font-bold tracking-tight text-fg ${wordmarkClassName ?? ""}`}
      >
        DELTA
      </span>
    </span>
